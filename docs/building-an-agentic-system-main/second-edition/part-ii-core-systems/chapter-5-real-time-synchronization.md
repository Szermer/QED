  ...
  return {
    observeSyncStatus(threadId: ThreadID): Observable<SyncStatus> {
      if (!statusTracking.has(threadId)) {
        statusTracking.set(
          threadId,
          new BehaviorSubject<SyncStatus>({
            state: "unknown",
            lastSync: null,
          }),
        );
...
export class BatchSyncOrchestrator {
  ...
  private async flushBatch(): Promise<void> {
    ...
    const batchEntries = Array.from(this.requestQueue.entries()).slice(
      0,
      this.MAX_BATCH_SIZE,
    );
...
export class OptimisticSyncManager {
  ...
  async waitForPendingUpdates(): Promise<void> {
    const promises = Array.from(this.pendingUpdates.values()).map(
...
export class SyncPerformanceMonitor {
  ...
  recordSyncTime(threadId: string, duration: number, size: number): void {
