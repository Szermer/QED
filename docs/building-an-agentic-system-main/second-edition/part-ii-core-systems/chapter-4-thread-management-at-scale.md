...
export class ThreadVersionControl {
  ...
  private threeWayMerge(base: Thread, local: Thread, remote: Thread): Thread {
...
export class ThreadSyncService {
  ...
  private async resolveConflict(local: Thread, remote: Thread): Promise<void> {
...
export class ThreadRelationshipManager {
  ...
  async getRelatedThreads(threadId: ThreadID): Promise<ThreadRelationships> {
    ...
    if (thread.summaryThreads) {
      relationships.summaries = await Promise.all(
        thread.summaryThreads.map((id) => this.threadService.getThread(id)),
      );
    }
...
export class ThreadDeltaCompressor {
  compress(oldThread: Thread, newThread: Thread): CompressedDelta {
  ...
  decompress(thread: Thread, delta: CompressedDelta): Thread {
    ...
    if (toFetch.length > 0) {
      ...
      for (const id of toFetch) {
        this.pendingReads.set(
          id,
          promise.then((batch) => batch.get(id)!),
        );
      }
